...
async def generate_comprehensive_summary_report(all_results: list, repo_urls: list = None):
    ...
    if total_files_reviewed > 0:
        print_and_capture(f"\nAGGREGATE CODE REVIEW METRICS:")
        print_and_capture("-" * 50)
        print_and_capture(f"Total Files Reviewed: {total_files_reviewed}")
        print_and_capture(f"Total Issues Found: {total_code_issues}")
        print_and_capture(f"Critical Issues: {total_critical_issues}")
        print_and_capture(f"Languages/Databases Analyzed: {', '.join(sorted(languages_reviewed))}")
            ...
            if 'pr_results' in result and result['pr_results']:
                ...
                for pr_idx, pr_result in enumerate(result['pr_results'], 1):
                    ...
                    if code_review and 'summary' in code_review:
                        summary = code_review['summary']
                        print_and_capture(f"\n        CODE REVIEW SUMMARY:")
                        print_and_capture(f"          Files Reviewed: {summary.get('files_reviewed', 0)}")
                        print_and_capture(f"          Total Issues: {summary.get('total_issues', 0)}")
                        print_and_capture(f"          Critical Issues: {summary.get('critical_issues', 0)}")
                        
                        # Show details by language/database
                        agent_results = code_review.get('agent_results', {})
                        if agent_results:
                            print_and_capture(f"\n          LANGUAGE/DATABASE BREAKDOWN:")
                            for agent_name, agent_data in agent_results.items():
                                if isinstance(agent_data, dict) and 'error' not in agent_data:
                                    files_analyzed = agent_data.get('files_analyzed', 0)
                                    if files_analyzed > 0:
                                        ...
                                        print_and_capture(f"            {lang}: {files_analyzed} files, {issues} issues ({critical} critical), quality: {quality}/100")
